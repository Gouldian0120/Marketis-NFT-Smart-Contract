...
//      nativeToken = 0x376495a6878A0E2B142c6bf5c0D29Fee785df5c9;

        nativeToken = 0xf7215a4C8A075B763deb3A8C3fAd8f6F08e83566;
    ...
    */
    ...
    function setNativeToken(address _newNativeToken) public virtual onlyOwner {
        nativeToken = _newNativeToken;
    }
    ...
    * @param _tokenId uint256 token ID (token number)
    */
    function purchaseTokenWithNRT(uint256 _tokenId, uint _amount) public nonReentrant {
        require(
            msg.sender != address(0) && msg.sender != ownerOf(_tokenId),
            'ERC721: Curent sender is already owner of this token'
        );
        
        require(_amount > _tokenMeta[_tokenId].price, 
            'ERC721: insufficient balance to purchase this NFT');
        
        require(IBEP20(nativeToken).balanceOf(msg.sender) >= _amount, 
            'ERC721: insufficient balance to purchase this NFT');
        ...
        IBEP20(nativeToken).safeTransferFrom(msg.sender, address(this), _amount);
        
        address tokenSeller = ownerOf(_tokenId);
        uint256 sellerProceeds = _tokenMeta[_tokenId].price;
        uint256 feesByTransfer = _payTxFeeWithNRT(_tokenId);
        
        IBEP20(nativeToken).safeTransfer(tokenSeller, sellerProceeds - feesByTransfer);
        
        _transfer(tokenSeller, msg.sender, _tokenId);
        _tokenMeta[_tokenId].sale = false;
    }
    
    function mint(address _owner, string memory _name, uint256 _price, uint256 _royalty, bool _sale) 
    ...
    function mintWithNRT(address _owner, string memory _name, uint256 _price, uint256 _royalty, bool _sale) 
        ...
        require(IBEP20(nativeToken).balanceOf(msg.sender) >= feeCreateToken, 
            'ERC721: insufficient balance to mint a new NFT');
        ...
        IBEP20(nativeToken).safeTransferFrom(msg.sender, owner(), feeCreateToken);
        
        return newItemId;
    }
    
    function mintCollectable(address _owner, string memory _name, uint256 _price, uint256 _royalty, bool _sale) 
    ...
    function _payTxFeeWithNRT(uint256 _tokenId) public returns (uint256) {
        // function _payTxFee(address from, uint256 _tokenId) internal returns (uint256) {
        // IERC20 token = IERC20(nativeToken);
        uint256 feesByTransfer = 0;
        ...
        if (tokenSeller != owner()) {
            ...
            IBEP20(nativeToken).safeTransfer(owner(), feeSellToken);
        ...
        if (tokenSeller != owner() && tokenSeller != artistOf(_tokenId)) {
            ...
            IBEP20(nativeToken).safeTransfer(artistOf(_tokenId), royaltyFee);
